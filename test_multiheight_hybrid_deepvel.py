...
from multiheight_hybrid_deepvel_torch import DeepVel_run
import pandas as pd
...
version = "v12"
checkpoints = [f"/home/xenoss/dat/thesis/models/{version}/hybrid_model/checkpoints/DeepVel_torch_epoch_97_0.21480.pt"]
model_names = ["Hybrid"]
...
csvs = [f"multiheight_stokes_hybrid_last2"]
common_csv = []
...
for i, ckpt in enumerate(checkpoints):
    ...
    deepvel_net = DeepVel_run(root = main_root, in_shape = input_shape, out_shape = output_shape, batch = 1, dataset_path = dataset_path, 
                              network_path = f"/home/xenoss/dat/thesis/models/{version}/hybrid_model/checkpoints/", test=True)
        
    params_model = ckpt

    input_path_I = test_path + f"inputs/stokes_I/"
    input_path_V = test_path + f"inputs/stokes_V/"
    

    input_I_data = os.listdir(input_path_I)
    input_I_data.sort()
    input_V_data = os.listdir(input_path_V)
    input_V_data.sort()

    metrics_rows = []
    for j in range(len(input_I_data)):
        input_I = torch.from_numpy(np.load(os.path.join(input_path_I, input_I_data[j])).astype(np.float32)).unsqueeze(0)
        input_V = torch.from_numpy(np.load(os.path.join(input_path_V, input_V_data[j])).astype(np.float32)).unsqueeze(0)
...
deepvel_net = DeepVel_run(root = main_root, in_shape=input_shape, out_shape=output_shape, batch = 1, dataset_path = dataset_path, network_path = model_path, test=True)
