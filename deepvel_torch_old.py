...
class dataset_deepVel(Dataset): 
    ...
    def __init__(self, dataset_path, test_idx = None, test = False):
        self.intensities_dir = os.path.join(dataset_path, "inputs/stokes_I") #TODO fix the hardcoding
    ...
    def check_indices (self, idx, test=False):
        ...
        intensity_index = self.intensities[idx].replace('stokes_I_', '')
...
class DeepVel_net(nn.Module):
    ...
    Model definition 
    """
    def __init__(self, in_channels=6, out_channels=2, n_filters=32, blocks=20, n_wavelengths=33): #TODO fix the hardcoding
...
class DeepVel_run(object):
    ...
    """
    def __init__(self, batch, dataset_path, network_path, in_channels = 2, root = None,n_wavelengths=33):
        ...
        self.model = DeepVel_net(in_channels=self.in_channels, out_channels=self.out_channels, n_filters=self.n_filters, blocks=self.n_conv_layers, n_wavelengths=self.n_wavelengths).to(device)
        ...
        if root:
            ...
            self.summary = summary(self.model, input_data = (torch.randn(self.batch_size, self.in_channels, self.n_wavelengths, 64, 64)), device=device)
...
if (__name__ == '__main__'):
    ...
    deepvel_net = DeepVel_run(root = main_root, in_channels=2, batch = 8, dataset_path = '/home/xenoss/dat/thesis/dataset/dataset_v1/', network_path = "/home/xenoss/dat/thesis/models/models_v1/stokes_i_model/checkpoints/")
